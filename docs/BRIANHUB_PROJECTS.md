...
- 登录、用户、模块权限、语言偏好和 AI API 参数由门户统一管理。
...
> 统一原则补充：登录、用户、模块权限、语言偏好、AI API 参数和企业微信通知参数均由门户统一管理；业务项目分别通过 `/internal/ai-config` 和 `/internal/notification-config` 读取集中配置，不在项目内重复保存密钥。
